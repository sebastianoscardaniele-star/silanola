...
const STORAGE_KEY = 'panini-render-directo-v12';
...
function getPresetPlayer(sectionCode, index) {
  return PLAYER_PRESETS[sectionCode]?.[index] || '';
}

function hydrateAlbumPlayers(album) {
...
function buildAlbum() {
  const fwc = {
    ...
    stickers: Array.from({ length: 19 }, (_, index) => ({
      ...
      code: `FWC-${index + 1}`,
      number: index + 1,
      player: FORCE_TEAMS[team.code]?.[index] || PLAYER_PRESETS[team.code]?.[index] || '',
      have: false,
      duplicates: 0
    }))
  };
  ...
  const countries = TEAMS.map(team => ({
    ...
    stickers: Array.from({ length: 20 }, (_, index) => ({
      ...
      number: index + 1,
      player: getPresetPlayer(team.code, index),
      have: false,
      duplicates: 0
    }))
  }));
...
function loadAlbum() {
  try {
    ...
    const album = saved ? JSON.parse(saved) : buildAlbum();
    if (!album || !Array.isArray(album.sections)) return hydrateAlbumPlayers(buildAlbum());
    return hydrateAlbumPlayers(album);
  } catch {
    return hydrateAlbumPlayers(buildAlbum());
  }
}
...
function forcedPlayer(sectionCode, stickerNumber, fallback = '') {
  return FORCE_TEAMS[sectionCode]?.[Number(stickerNumber) - 1] || fallback || '';
}
                  ...
                  <article className={sticker.have ? 'sticker owned' : 'sticker'} key={sticker.id}>
                    ...
                    {section.type === 'team' && (
                      <label className="player-field">
                        Jugador
                        <input
                          value={forcedPlayer(section.code, sticker.number, sticker.player)}
