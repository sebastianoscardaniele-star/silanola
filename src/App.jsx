...
const STORAGE_KEY = 'panini-2026-jugadores-visibles-v3';
const UI_STORAGE_KEY = 'panini-2026-pro-ui-preferencias-v1';
    ...
    sections: album.sections.map(section => {
      ...
      return {
        ...
        stickers: section.stickers.map((sticker, index) => ({
          ...sticker,
          player: sticker.player && sticker.player.trim() ? sticker.player : (preset[index] || '')
        }))
      };
    })
  };
...
function loadAlbum() {
  try {
    ...
    if (!saved) return mergePlayerPresets(buildAlbum());
    const parsed = JSON.parse(saved);
    if (!parsed || !Array.isArray(parsed.sections)) return mergePlayerPresets(buildAlbum());
    return mergePlayerPresets(parsed);
  } catch {
    return buildAlbum();
  }
}
