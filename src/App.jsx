...
const STORAGE_KEY = 'panini-2026-all-players-stable-v10';
...
function buildAlbum() {
  ...
  const countries = TEAMS.map(team => ({
    ...
    stickers: Array.from({ length: 20 }, (_, index) => ({
      ...
      number: index + 1,
      player: PLAYER_PRESETS[team.code]?.[index] || '',
      have: false,
      duplicates: 0
    }))
  }));
...
function applyPlayerPresets(album) {
    ...
    sections: album.sections.map(section => {
      ...
      return {
        ...
        stickers: section.stickers.map((sticker, index) => ({
          ...sticker,
          player: preset[index] || sticker.player || ''
        }))
      };
    })
  };
...
function loadAlbum() {
  try {
    ...
    const baseAlbum = saved ? JSON.parse(saved) : buildAlbum();
    if (!baseAlbum || !Array.isArray(baseAlbum.sections)) return applyPlayerPresets(buildAlbum());
    return applyPlayerPresets(baseAlbum);
  } catch {
    return applyPlayerPresets(buildAlbum());
  }
}
...
export default function App() {
  ...
  useEffect(() => {
    setAlbum(current => applyPlayerPresets(current));
  }, []);
